/* ═══════════════════════════════════════════════════════
   OSTP DevTool — Exporter Module (PATCHED v2: Incluye vacíos)
...
export function buildMd() {
  ...
  // ── PATCH v2: Incluir TODOS los archivos ──
  ...
  if (items.length) {
    ...
    items.forEach(item => {
      ...
      const contentDisplay = item.content && item.content.trim() ? item.content : `/* ⚠️ ARCHIVO VACÍO — ${(item.file.size)} bytes */\n`;
